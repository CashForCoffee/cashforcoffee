...
function tableHTML(items,mode='period',periodIdx=null){
  ...
  ${displayItems.length?displayItems.map((i,index)=>`<tr class="${mode==='all'&&isMainIncome(i)&&index>0?'pay-period-start':''}" data-search="${esc([i.item,i.category,i.notes,i.type,i.date].join(' ').toLowerCase())}" onclick="editItem('${i.id}')">
    <td data-label="Date"><span class="date-line">${fullDate(i.date)}${showContext&&dateContext(i.date)?`<span class="date-context">${dateContext(i.date)}</span>`:''}</span></td><td data-label="Item"><span class="item-main ${i.paid?'paid':''}">${esc(i.item)}</span>${i.notes?`<div style="font-size:10px;color:var(--muted);margin-top:3px">${esc(i.notes)}</div>`:''}</td>
...
function allView(){const items=allItems(),income=state.items.filter(i=>i.type==='Income').reduce((s,i)=>s+i.amount,0),savings=state.items.filter(isSavingsItem).reduce((s,i)=>s+i.amount,0),outgoings=state.items.filter(i=>i.type!=='Income').reduce((s,i)=>s+i.amount,0),unpaid=state.items.filter(i=>i.type!=='Income'&&!i.paid).reduce((s,i)=>s+i.amount,0);return `<div class="all-grid"><section class="all-summary"><div class="mini-card"><div class="label">All items</div><div class="value mono">${state.items.length.toLocaleString()}</div></div><div class="mini-card"><div class="label">Total income</div><div class="value mono" style="color:var(--green)">${money(income)}</div></div><div class="mini-card"><div class="label">Total outgoings</div><div class="value mono" style="color:var(--red)">${money(outgoings)}</div></div><div class="mini-card"><div class="label">Savings</div><div class="value mono" style="color:var(--amber)">${money(savings)}</div></div><div class="mini-card"><div class="label">Unpaid outgoings</div><div class="value mono" style="color:var(--blue)">${money(unpaid)}</div></div></section><section class="panel"><div class="panel-head"><div><div class="panel-title">Everything</div><div class="panel-sub">Every imported and manually added budget item</div></div><div class="panel-spacer"></div><button class="primary" onclick="addItem()">＋ Add item</button></div><div class="toolbar"><input class="search" placeholder="Search all items…" value="${esc(state.allSearch)}" oninput="setAllSearch(this.value)"><select class="select" onchange="setAllCategory(this.value)">${categoryOptions(state.allCategory)}</select><select class="select" onchange="setAllPaid(this.value)"><option value="all">All</option><option value="unpaid" ${state.allPaid==='unpaid'?'selected':''}>Unpaid</option><option value="paid" ${state.allPaid==='paid'?'selected':''}>Paid</option></select></div>${tableHTML(items,'all')}</section></div>`}
