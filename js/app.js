...
window.togglePaid=async(id,paid)=>{const item=state.items.find(i=>i.id===id);if(!item||state.pending.has(id))return;const scrollPos=captureScrollState();state.pending.add(id);item.paid=!paid;state.sync='saving';render();restoreScrollState(scrollPos);const{error}=await sb.from('budget_items').update({paid:!paid}).eq('id',id);state.pending.delete(id);if(error){const rollbackPos=captureScrollState();item.paid=paid;state.sync='error';render();restoreScrollState(rollbackPos);fail(error,'Could not update item')}else{state.sync='';renderTopSync()}};
