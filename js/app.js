...
let state={items:[],settings:null,session:null,authMode:'login',itemsLoaded:false,view:'dashboard',selectedPeriod:null,search:'',filter:'all',category:'all',type:'all',allSearch:'',allCategory:'all',allType:'all',allPaid:'all',modal:null,tab:'settings',editing:null,importParsed:null,busy:false,sync:'',pending:new Set(),bulkScope:'future'};
...
function summaryCards(p){const s=periodStats(p);return `<section class="summary">
  ...
  <div class="summary-card expense"><div class="summary-label">↧ Outgoings</div><div class="summary-value mono">${money(s.outgoings)}</div><div class="summary-note">Including savings</div></div>
  <div class="summary-card remaining"><div class="summary-label">＝ Remaining outgoings</div><div class="summary-value mono">${money(s.remainingOutgoings)}</div><div class="summary-note">Still to leave the main account</div></div>
...
function tableHTML(items,mode='period',periodIdx=null){
  ...
  ${displayItems.length?displayItems.map((i,index)=>`<tr class="${mode==='all'&&isMainIncome(i)&&index>0?'pay-period-start':''}" data-search="${esc([i.item,i.category,i.notes,i.type,i.date].join(' ').toLowerCase())}" onclick="editItem('${i.id}')">
    <td data-label="Date"><span class="date-line">${fullDate(i.date)}${showContext&&dateContext(i.date)?`<span class="date-context">${dateContext(i.date)}</span>`:''}</span></td><td data-label="Item"><div class="item-line"><span class="item-main ${i.paid?'paid':''}">${esc(i.item)}</span><span class="pill mobile-category ${categoryClass(i.category)}">${esc(i.category)}</span></div>${i.notes?`<div class="item-notes">${esc(i.notes)}</div>`:''}</td>
...
function render(){if(!state.settings)return;document.body.classList.toggle('dashboard-lock',state.view==='dashboard');document.body.classList.toggle('all-lock',state.view==='all');const ps=buildPeriods(),p=selectedPeriod(ps);if(state.selectedPeriod===null)state.selectedPeriod=p.idx;const[title,sub]=pageMeta();$('#app').innerHTML=`<div class="shell"><aside class="sidebar"><div class="brand">${brandmark()}<div><div class="brandname">CashForCoffee</div><div class="brandsub">Our household budget</div></div></div><nav class="nav-group"><button class="nav-btn ${state.view==='dashboard'?'active':''}" onclick="setView('dashboard')"><span class="nav-icon">⌂</span>Dashboard</button><button class="nav-btn ${state.view==='all'?'active':''}" onclick="setView('all')"><span class="nav-icon">☷</span>All items</button><button class="nav-btn ${state.view==='forecast'?'active':''}" onclick="setView('forecast')"><span class="nav-icon">▥</span>Forecast</button><button class="nav-btn ${state.view==='breakdown'?'active':''}" onclick="setView('breakdown')"><span class="nav-icon">◔</span>Breakdown</button><button class="nav-btn ${state.view==='cashflow'?'active':''}" onclick="setView('cashflow')"><span class="nav-icon">⌁</span>Cash flow</button><button class="nav-btn" onclick="openData()"><span class="nav-icon">⚙</span>Manage</button></nav><div class="side-label">Quick actions</div><button class="quick-btn" onclick="addItem()">⊕ Add item</button><button class="quick-btn" onclick="openData('bulk')">↻ Bulk update</button><button class="quick-btn" onclick="openData('import')">⇩ Import / Fresh start</button><button class="quick-btn" onclick="openData('export')">⇧ Export / Backup</button><button class="quick-btn" onclick="markPeriodPaid()">✓ Mark period paid</button><div class="side-spacer"></div></aside><main class="main"><header class="topbar"><div class="mobile-brand">${brandmark()}<div><div class="brandname">CashForCoffee</div><div class="brandsub">${esc(sub)}</div></div></div><div class="page-heading"><div class="page-title">${esc(title)}</div><div class="page-sub">${esc(sub)}</div></div><div class="top-spacer"></div><button class="period-picker" onclick="setView('forecast')"><span class="icon">📅</span><span><span class="label">${p.idx===currentIdx()?'Current':'Selected'} fortnight</span><span class="value">${esc(periodLabel(p))}</span></span><span style="margin-left:auto;color:var(--faint)">⌄</span></button><button class="top-manage" aria-label="Manage" title="Manage" onclick="openData()">⚙</button><div class="sync-state"></div></header><div class="content">${state.view==='dashboard'?dashboardView(ps,p):state.view==='all'?allView():state.view==='forecast'?forecastView(ps,p):state.view==='breakdown'?breakdownView():cashflowView()}</div></main></div><button class="fab" onclick="addItem()">＋</button><nav class="mobile-nav"><button class="${state.view==='dashboard'?'active':''}" onclick="setView('dashboard')"><span>⌂</span><span>Dashboard</span></button><button class="${state.view==='all'?'active':''}" onclick="setView('all')"><span>☷</span><span>All items</span></button><button class="${state.view==='forecast'?'active':''}" onclick="setView('forecast')"><span>▥</span><span>Forecast</span></button><button class="${state.view==='cashflow'?'active':''}" onclick="setView('cashflow')"><span>⌁</span><span>Cash flow</span></button><button class="${state.view==='breakdown'?'active':''}" onclick="setView('breakdown')"><span>◔</span><span>Breakdown</span></button></nav>`;renderTopSync();renderModal();syncMobileTopbarHeight();if(state.view==='cashflow')requestAnimationFrame(()=>renderCashflowChart(ps))}
...
function syncMobileTopbarHeight(){const bar=document.querySelector('.topbar');if(!bar)return;const set=()=>document.documentElement.style.setProperty('--mobile-topbar-height',`${Math.ceil(bar.getBoundingClientRect().height)}px`);set();if(window.ResizeObserver){window.__cfcTopbarObserver?.disconnect?.();window.__cfcTopbarObserver=new ResizeObserver(set);window.__cfcTopbarObserver.observe(bar)}}
